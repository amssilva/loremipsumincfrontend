...
export default {
  ...
  created(){
    axios.get(`${utils.getServiceUrl()}/projetos/${this.$route.params.id}/`).then(response =>{
  ...
  methods:{
    ...
    salvar(event) {
      ...
      axios.put(`${utils.getServiceUrl()}/projetos/${this.$route.params.id}/`, this.model)
        .then(() =>{
          this.$root.$bvToast.toast('Projeto atualizado com sucesso', {title: 'Notificação',variant: 'success',solid: true})
          this.$router.push('/');
        })
        .catch(() =>{
